...
| PDF請求書明細抽出（複数商品対応） | 必須     | 自動                 |
| PDF表領域検出＆汎用テーブルパーサー    | 必須     | 自動                 |
| 在庫確認                    | 必須     | 自動                 |
...
| order_id (タイトル) | customers (Relation) | total_price (数値) | delivery_date (日付) | status (選択: approved, cancel, unprocessed, completed) | approved_by (テキスト) | created_at (作成日時) |
| ------------------- | ------------------- | ----------------- | ------------------- | ------------------------------------------------------- | -------------------- | -------------------- |
| ORD001              | CUST001              | 294000            | 2025-07-15          | approved                                                 | tanaka               | 2025-07-12T10:00:00  |

### 注文明細（order_details）

| id (タイトル) | orders (Relation) | products (Relation) | quantity (数値) | sub_total (数値) | created_at (作成日時) |
| ------------- | ----------------- | ------------------- | -------------- | --------------- | -------------------- |
| DTL001        | ORD001            | A001                | 3              | 294000           | 2025-07-12T10:01:00  |
...
- フォーマット多様性対応：PDF請求書／注文書のレイアウト揺らぎを吸収するための表領域検出＆抽出ロジック強化
