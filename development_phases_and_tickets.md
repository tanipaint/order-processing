...
|     2–7      | マルチ商品注文抽出対応                    | PDFテーブル抽出を優先し、本文メタデータ抽出へフォールバックするパイプラインを実装。複数商品のitemsリストを確実に返却し、`doc/001_purchase_order.pdf` のE2Eテストを追加 | backend  |  ✔️  |
...
|    2–12      | OCRテキスト調査＆正規表現拡張               | 実際のOCR抽出テキストをログ出力し、変則フォーマット用の正規表現パターンを収集 | backend  |      |
|    2–13      | LLM抽出プロンプト A/B テスト                | 異なるプロンプト案を比較し、多商品抽出精度を測定するテストを実施             | backend  |      |
